...
object LocationParser {
    ...
    fun parse(text: String): SharedLocation {
        ...
        val dirPts = url?.let { extractDirWaypoints(it) }.orEmpty()
        val waypoints = if (dirPts.size >= 2) dirPts.dropLast(1).drop(1) else emptyList()
        val destFromDir = dirPts.lastOrNull()
        ...
        return SharedLocation(
            name = name,
            lat = destFromDir?.first ?: coords?.first,
            lng = destFromDir?.second ?: coords?.second,
            url = url,
            needsExpansion = (coords == null && destFromDir == null && url != null),
            waypoints = waypoints,
        )
    }
    ...
    private val coordDirSeg = Regex("/dir/([^?]+)")

    /** Parse `/dir/lat,lng/lat,lng/...` segments from Google Maps directions URLs. */
    fun extractDirWaypoints(s: String): List<Pair<Double, Double>> {
        val path = coordDirSeg.find(s)?.groupValues?.get(1) ?: return emptyList()
        return path.split('/').mapNotNull { seg ->
            val parts = seg.split(',')
            if (parts.size < 2) return@mapNotNull null
            val lat = parts[0].toDoubleOrNull() ?: return@mapNotNull null
            val lng = parts[1].toDoubleOrNull() ?: return@mapNotNull null
            if (valid(lat, lng)) lat to lng else null
        }
    }

    /** From a directions URL, return (waypoints excluding dest, destination). */
    fun parseDirections(s: String): Pair<List<Pair<Double, Double>>, Pair<Double, Double>?> {
        val pts = extractDirWaypoints(s)
        if (pts.size < 2) return emptyList<Pair<Double, Double>>() to pts.lastOrNull()
        return pts.dropLast(1).drop(1) to pts.last()
    }
