...
class DashSession(private val scope: CoroutineScope) {
    companion object {
        ...
        private const val NAV_INFO_MS    = 500L   // 2 Hz nav bubble — snappier distance updates
        private const val ART_PACKET_DELAY_MS = 10L
        ...
        private const val HOSTNAME       = "TripperMate"
    }
    ...
    @Volatile private var npPlaying = false
    @Volatile private var npArtJpeg: ByteArray? = null
    @Volatile private var caller: String? = null

    /** Push the latest now-playing metadata; sent to the dash at ~1 Hz while streaming. */
    fun updateNowPlaying(title: String?, album: String, artist: String, playing: Boolean = true, artJpeg: ByteArray? = null) {
    ...
    fun updateRouteCard(name: String) {
        destinationName = name.ifBlank { "TripperMate" }
    ...
    /** Re-push route card + nav bubble after a late-arriving OSRM route lands. */
    fun refreshNavPresentation() {
        if (_state.value != DashState.STREAMING) return
        scope.launch(Dispatchers.IO) {
            socket?.send(liveRouteCard(projectionOn = true))
        }
    }
    ...
    /**
     * Now-playing (05 0d) + album art (05 40) + incoming-call (05 22) keep-alive, ~1 Hz.
     */
    private fun launchMediaInfo() {
        ...
        mediaInfoJob = scope.launch(Dispatchers.IO) {
            ...
            while (isActive && _state.value == DashState.STREAMING) {
                ...
                when {
                    c != null -> {
                        runCatching { sock.send(DashCommands.callNotify(c)) }
                        if (!loggedCall) { loggedCall = true
                            com.example.northstar.data.RideDiagnostics.log("media", "TX call card 05 22: $c") }
                    }
                    prevCaller != null -> { runCatching { sock.send(DashCommands.callClear()) }; loggedCall = false }
                }
                prevCaller = c
                npTitle?.let { title ->
                    runCatching { sock.send(DashCommands.musicWidgetOn()) }
                    runCatching { sock.send(DashCommands.musicPlayState(npPlaying)) }
                    runCatching { sock.send(DashCommands.nowPlaying(title, npAlbum, npArtist)) }
                    if (!loggedNp) { loggedNp = true
                        com.example.northstar.data.RideDiagnostics.log("media", "TX now-playing 05 0d: $title") }
                    val art = npArtJpeg
                    if (art != null && art.isNotEmpty()) {
                        val hash = art.contentHashCode()
                        if (hash != lastArtHash) {
                            lastArtHash = hash
                            sendAlbumArt(sock, art)
                        }
                    }
                }
                delay(ROUTE_CARD_MS)
    ...
    private suspend fun sendAlbumArt(sock: DashSocket, jpeg: ByteArray) {
        delay(50)
        for (pkt in DashCommands.albumArtPackets(jpeg)) {
            runCatching { sock.send(pkt) }
            delay(ART_PACKET_DELAY_MS)
        }
        com.example.northstar.data.RideDiagnostics.log("media", "TX album art 05 40 (${jpeg.size}B, ${DashCommands.albumArtPackets(jpeg).size} pkts)")
    }
