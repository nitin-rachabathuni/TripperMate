...
object DashCommands {
    ...
    fun routeCard(
        ...
        // Always overwrite the template's captured roundabout glyph (0x3C) — leaving it causes
        // every turn to show as a roundabout when live nav values aren't patched yet.
        patch1(0x05, 0x02, maneuver ?: NAV_MANEUVER_CONTINUE)
        primaryDist?.let { patch2(0x05, 0x04, it) }
    ...
    /** 05 17 55 — activate the dash's native music widget before metadata/art. */
    fun musicWidgetOn() = K1GPacket.build(K1GPacket.tlv(0x05, 0x17, 0x55))

    /** 05 19 play-state: 0x55 = playing, 0xAA = paused. */
    fun musicPlayState(playing: Boolean) =
        K1GPacket.build(K1GPacket.tlv(0x05, 0x19, if (playing) 0x55 else 0xAA))

    private const val ART_SIZE = 75
    private const val ART_JPEG_QUALITY = 85
    private const val ART_CHUNK_SIZE = 1000

    /**
     * Build chunked 05 40 album-art packets (TLV JPEG fragments for the dash music widget).
     * Sequence: first chunk (0x00 + total size + bytes) → middle (index + bytes) → last (0xFF + bytes).
     */
    fun albumArtPackets(jpeg: ByteArray, chunkSize: Int = ART_CHUNK_SIZE): List<ByteArray> {
        if (jpeg.isEmpty()) return emptyList()
        val chunks = jpeg.toList().chunked(chunkSize)
        val total = jpeg.size
        val out = ArrayList<ByteArray>(chunks.size + 1)
        chunks.forEachIndexed { i, part ->
            val payload = ByteArrayOutputStream()
            when {
                chunks.size == 1 -> {
                    payload.write(0x00)
                    payload.write((total shr 8) and 0xFF)
                    payload.write(total and 0xFF)
                    payload.write(part.toByteArray())
                    out.add(K1GPacket.build(K1GPacket.tlv(0x05, 0x40, payload.toByteArray())))
                    out.add(K1GPacket.build(K1GPacket.tlv(0x05, 0x40, byteArrayOf(0xFF.toByte()))))
                }
                i == 0 -> {
                    payload.write(0x00)
                    payload.write((total shr 8) and 0xFF)
    ...
    /** Scale/compress album art for the dash music widget. */
    fun prepareAlbumArtJpeg(source: android.graphics.Bitmap): ByteArray {
        val thumb = android.media.ThumbnailUtils.extractThumbnail(source, ART_SIZE, ART_SIZE)
        val out = java.io.ByteArrayOutputStream()
        thumb.compress(android.graphics.Bitmap.CompressFormat.JPEG, ART_JPEG_QUALITY, out)
        if (thumb !== source) thumb.recycle()
        return out.toByteArray()
    }

    /**
     * 05 40 album-art frames (legacy simple chunker — prefer [albumArtPackets]).
     */
    fun albumArtFrames(jpeg: ByteArray, chunk: Int = 1000): List<ByteArray> =
        albumArtPackets(jpeg, chunk)
