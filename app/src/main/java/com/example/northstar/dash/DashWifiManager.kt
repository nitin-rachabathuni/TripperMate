...
class DashWifiManager(
    ...
    fun connect(ssid: String, password: String = "", prefixMatch: Boolean = false) {
        ...
        // OEM fallback (ColorOS/MIUI/HyperOS/Motorola): if the rider already joined the
        // dash hotspot in system Wi‑Fi settings, skip WifiNetworkSpecifier — many OEM skins
        // suppress that dialog or block requestNetwork() for no-internet hotspots.
        val prefix = if (prefixMatch) ssid else ssid.substringBefore('_') + "_"
        findExistingDashNetwork(prefix)?.let { (network, resolved) ->
            Log.i(TAG, "Using existing OS Wi‑Fi connection to '$resolved' (OEM fallback)")
    ...
    /** True when [ssid] is too short to be a real dash name (e.g. stored prefix only). */
    fun isIncompleteSsid(ssid: String, prefix: String = DashConfig.DEFAULT_PREFIX): Boolean =
        ssid.isBlank() || ssid == prefix || ssid.length <= prefix.length + 2

    /**
     * If the phone is already on a dash hotspot at the OS level, return its [Network] and SSID.
     * Used before [requestNetwork] on OEM skins that block programmatic joins.
     */
    @SuppressLint("MissingPermission")
    fun findExistingDashNetwork(prefix: String): Pair<Network, String>? {
        return try {
            for (network in cm.allNetworks) {
                ...
                if (ssid.isBlank() || ssid == WifiManagerUnknownSsid) continue
                if (!ssid.startsWith(prefix)) continue
                Log.i(TAG, "Found existing dash network: '$ssid' on $network")
                return network to ssid
            }
            null
        } catch (e: Exception) {
            Log.w(TAG, "findExistingDashNetwork failed: ${e.message}")
            null
        }
    }

    /** Bind to an already-connected dash network without calling [requestNetwork]. */
    fun adoptExistingNetwork(network: Network, ssid: String) {
        wantConnected = true
        everConnected = true
        this.network = network
        resolvedSsid = ssid
