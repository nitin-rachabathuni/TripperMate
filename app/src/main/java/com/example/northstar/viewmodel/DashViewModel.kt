...
class DashViewModel(app: Application) : AndroidViewModel(app) {
    ...
    @Volatile private var routeWaypoints: List<GeoPoint> = emptyList()
    ...
    companion object {
        ...
        private const val BTN_RECENTER   = 0x22   // long-press / home — recenter + default zoom
        private const val DEFAULT_ZOOM = 19
        private const val FRAME_MS_HOT   = 1_000L // 1 fps when thermally hot
    }

    @Volatile private var thermalLevel = 0  // 0=OK, 1=warm, 2=hot+
    ...
    init {
        ...
        // Retry routing when GPS arrives after the rider sent a destination (fixes "slow GPS / no route").
        viewModelScope.launch {
            location.location.collect { loc ->
                if (loc != null && destLat != null && destLng != null && route == null && !routeFetching) {
        ...
        session.onButton = { btn ->
            ...
            val label = when {
                ...
                !ringing && code == BTN_RECENTER   -> { recenter(resetZoom = true); "Recenter + default zoom" }
    ...
    fun connect() {
        ...
        // If a partial SSID (e.g. "RE_") was stored from a failed OEM connect, rediscover.
        if (wifiManager.isIncompleteSsid(dashConfig.ssid, dashConfig.ssidPrefix)) {
            dashConfig.forgetDash()
        }

        // OEM fallback: phone already joined the dash hotspot in system settings.
        wifiManager.findExistingDashNetwork(dashConfig.ssidPrefix)?.let { (network, ssid) ->
            dashConfig.ssid = ssid
            _ui.update { it.copy(ssid = ssid) }
            wifiManager.adoptExistingNetwork(network, ssid)
            session.connect(ssid, network)
            return
        }
    ...
    fun setDestination(name: String, lat: Double?, lng: Double?, waypoints: List<Pair<Double, Double>> = emptyList()) {
        routeWaypoints = waypoints.map { (a, b) -> GeoPoint(a, b) }
        _ui.update { it.copy(
    ...
    fun exitNavigation() {
        ...
        session.updateRouteCard("TripperMate")   // dash card → name + 0.0 km, nav off
    }
    ...
    private fun fetchRoute(destLatV: Double, destLngV: Double) {
        ...
        viewModelScope.launch {
            ...
            if (r != null) {
                ...
                session.refreshNavPresentation()
                lastSignature = ""
    ...
    fun resetZoom() = setZoom(DEFAULT_ZOOM)
    ...
    fun recenter(resetZoom: Boolean = false) {
        panX = 0f; panY = 0f; followMode = true
        if (resetZoom) zoom = DEFAULT_ZOOM
        _ui.update { it.copy(followMode = true, mapZoom = if (resetZoom) DEFAULT_ZOOM else _ui.value.mapZoom) }
        lastSignature = ""
    }
    fun toggleHeadingUp() {
        headingUp = !headingUp
        ...
        streamJob = viewModelScope.launch(Dispatchers.Default) {
            ...
            while (isActive && session.state.value == DashState.STREAMING) {
                try {
                    ...
                    frameMs = when (thermalLevel) {
                        2 -> FRAME_MS_HOT
                        1 -> FRAME_MS_IDLE
                        else -> if (active) FRAME_MS_ACTIVE else FRAME_MS_IDLE
                    }
                    val bmp = frameBitmap
                    val enc = encoder
    ...
    private fun tick(): Boolean {
        ...
        if (r != null && loc != null) {
            ...
            val glyph = ns.nextManeuver?.dashCode ?: DashCommands.NAV_MANEUVER_CONTINUE
            session.updateNavInfo(glyph, pv, pu, tv, tu, etaHHMM)
            maneuverText = ns.nextManeuver?.instruction
        ...
        mediaObserveJob = viewModelScope.launch {
            launch {
                mediaInfo.nowPlaying.collect { np ->
                    ...
                    val art = np?.art?.let { com.example.northstar.dash.protocol.DashCommands.prepareAlbumArtJpeg(it) }
                    val playing = np?.playing == true
                    session.updateNowPlaying(np?.title, np?.album.orEmpty(), np?.artist.orEmpty(), playing, art)
                }
            }
            launch {
