...
class RouteViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun handleSharedText(text: String) {
        ...
        if (loc.lat != null && loc.lng != null) {
            computeRoute(loc.lat, loc.lng, loc.waypoints)
            ...
            viewModelScope.launch {
                ...
                val dirPts = if (urlCoords == null) LocationParser.extractDirWaypoints(loc.url) else emptyList()
                val wps = if (dirPts.size >= 2) dirPts.dropLast(1).drop(1) else loc.waypoints
                val dest = urlCoords ?: dirPts.lastOrNull()
                val resolved = loc.copy(
                    name = name,
                    lat = dest?.first ?: coords?.first,
                    lng = dest?.second ?: coords?.second,
                    needsExpansion = false,
                    waypoints = wps,
                )
                _state.value = _state.value.copy(destination = resolved, isResolving = false)
                val dLat = resolved.lat; val dLng = resolved.lng
                if (dLat != null && dLng != null) computeRoute(dLat, dLng, wps)
    ...
    private fun computeRoute(destLat: Double, destLng: Double, waypoints: List<Pair<Double, Double>> = emptyList()) {
        ...
        viewModelScope.launch {
            val via = waypoints.map { (lat, lng) -> GeoPoint(lat, lng) }
            val r = Router.route(
